...
class TestVoltronAzureServiceBusQueue(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.sample_voltron_payload = {'handlerName':'samplehandler','handlerconfig':{},'handlerData':{},'messageSource':'test_voltron_azure.py','startTime':12345}
        self.sample_received_messages = [ServiceBusMessage(json.dumps(self.sample_voltron_payload))]

    @mock.patch("src.voltronsecurity.voltron_azure.ServiceBusClient", return_value=mock.AsyncMock())
    ...
    def test_happy_default_handle_messages(self, mock_sbc):
        ...
        handler = VoltronAzureServiceBusQueue(self.sample_queue_name, self.sample_namespace, self.sample_creds)
        resp = asyncio.run(handler.handle_messages())
        mock_sbr.complete_message.assert_called()
    
    @mock.patch("src.voltronsecurity.voltron_azure.VoltronAzureServiceBusQueue.process_message", return_value={"success": False, "message": "MockedFail", "data": {}})
    @mock.patch("src.voltronsecurity.voltron_azure.ServiceBusClient")
    ...
    def test_generate_message(self):
        ...
        new_message = handler.generate_message(msg_h_name, msg_h_config, msg_h_data, msg_source, msg_start_time)
